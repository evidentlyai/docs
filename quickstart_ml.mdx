...
include_tests="True")
my_eval = report.run(adult_prod, adult_ref)
```

**Note**: in this simple example we directly work with pandas dataframes, but it is recommended to create an Evidently Dataset Object and add [data definition](/docs/library/data_definition) to specify column types.
