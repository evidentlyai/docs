...
In this tutorial, you will learn how to set up tracing for an LLM application to collect its inputs and outputs, view the traces in Evidently Cloud, and optionally run evaluations.
...
* `Project_id` should belong to the Evidently Project you just created. Go to the [Projects page](https://app.evidently.cloud/), enter the Project and copy its ID from above the dashboard.

* `Dataset_name` helps identify the resulting Tracing dataset. All data with the same ID would be grouped into a single dataset.
...
You can copy it from the dataset ID from the Traces page inside your Project.
...
To summarize and run conditional checks:
...
This will explicitly test if all responses are 1 sentence, and each one's length is \<300 symbols.
...
Check the tutorial on [LLM evaluations](/quickstart_llm) for more details: how to run other evaluation methods, including LLM as a judge, or test for specific conditions. 
