...
**Add scores when creating Dataset**. If you have a dataframe `df` with text data, you can create a `Dataset`, set the [Data Definition](/docs/library/data_definition) and add descriptors for the "answer" column at once:
...
Under the hood, the `TextEvals` Preset generates a `ValueStats` Metrics for each Descriptor. To have more control or use other available Metrics, you can create a custom Report referencing descriptors just like any other column in the dataset.
