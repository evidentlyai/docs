...
<Note>
  **List of all Metrics**. Check the [Reference table](/metrics/all_metrics). Consider column-level Metrics like `MeanValue`, `MeanValue`, `MaxValue`, `QuantileValue`, `OutRangeValueCount() for numerical and `CategoryCount` for categorical.
</Note>