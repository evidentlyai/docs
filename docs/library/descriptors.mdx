...
**Note**. Some Descriptors (like `OOVWordsPercentage()` for out-of-vocabulary words) may require downloading `nltk` dictionaries:
...
**Option A. Add descriptors when creating Dataset**. Create a `Dataset` from a dataframe `df`, set the [Data Definition](/docs/library/data_definition) and add descriptors at once. You must specify the column you are generating scores for each descriptor.
...
**Option B. Add scores to existing Dataset**. You can also add descriptors to the Dataset object later using `add_descriptors`. For example, first create the Dataset.
...
**Step 3. (Optional). Export results**. You can get the Dataset with newly added descriptors as a DataFrame:
...
**Step 4. Get the Report**. Use Reports to summarize the results: see stats for all descriptors, visualize distributions and run conditional tests. The easiest way to get the Report is through `TextEvals` Preset: it instantly summarizes all results.
...
**Custom evals**. Beyond custom LLM judges, you can also implement your own programmatic evals. Check the [in-depth LLM judge guide](/metrics/customize_llm_judge) on using built-in and custom LLM-based evaluators.
...
**Step 2. Add tests to a descriptor**. When creating a descriptor (like `TextLength` or `Sentiment`), use the tests argument to define conditions. Each condition adds a new column with a True/False result.
