...
Check the Quickstart [for ML](/quickstart_ml) or [LLM](/quickstart_llm) for a simple example.
...
**Two datasets**. To generate the Data Drift Report that needs two datasets, pass the second one as a reference:
