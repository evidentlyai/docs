...
* Pass the corresponding `data_definition`.
...
**Automated data definition.**  You can often use automated mapping. If the data definition is not specified, Evidently tries to map columns:
...
Map the types of the inputs columns. You only need to map the ones you plan to use in your evaluations. Knowing the column type helps Evidently compute correct statistics, visualizations, and pick default tests.
...
<Info>
**It's optional but useful**. For **LLM evals**, you can [generate text descriptors](/docs/library/descriptors) without explicitly mapping text columns. But it's a good idea to map them since you may later run dataset-level evaluations which vary by column type (like Dataset Summary).
</Info>
...
However, if you computed some scores or metadata externally and want to treat them as decriptors, you can map them explicitly:
