...
**Prepare your data**. You can pass data with [flexible structure](/docs/library/overview#dataset). Prepare it as a pandas.DataFrame.
...
<Info>
  **It's optional but useful**. You can [generate text descriptors](/docs/library/descriptors) without explicitly mapping text columns. But it's a good idea to map them since you may later run other evaluations which vary by column type (like Dataset Summary).
</Info>
