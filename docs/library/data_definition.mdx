...
You only need to map columns that will be used in your evaluations. This lets Evidently correctly process the data schema and know how to treat specific columns when computing the Metrics you later select. Some types of evaluations require certain columns present and will they fail otherwise.

You can specify the `DataDefinition` in Python prior to generating a Report or map the columns visually when working in the Evidently platform. In some cases, you can use **automated data definition.**&#x20;
...
If your have your data in a pandas.DataFrame `source_df ` create a`Dataset` from pandas and specify the `data_definition`

With **automated** column type recognition:
...
With **user-defined** column types:
...
You can map column roles similarly: see examples below based on the use case.&#x20;
