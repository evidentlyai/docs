...
* Prepare your data as a pandas.DataFrame (or two).

* Create an Evidently Dataset object using `Dataset.from_pandas`.

* Pass the `data_definition`, including the necessary details.

To use automated mapping, pass an empty `DataDefinition()`object:
...
**Automated data definition.**  If DataDefinition is not specified, Evidently will try to map columns automatically:

* Based on type (numerical, categorical).

* By matching column names to known roles (e.g., a column named "target" will be treated as the target).

**Manual data definition.** While automation works in many cases, manual mapping is more accurate. It helps avoid mistakes like misclassifying numerical columns with few unique values as categorical. For some evaluations, like drift detection with text or embedding columns, explicit mapping is required.

* Only map what you need for your evaluation. For example, you don’t need to set any any column roles to run data quality checks, but mapping type is useful.

* If you map columns manually and exlcude certain columns, they will be ignored in evaluations even if present in the dataset.
...
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| `numerical_columns`   | <ul><li>Columns with numeric values.</li><li>Mapping affects choice of statistics, automated tests, data drift detection methods and visualizations.</li></ul>                                                                                                                                                                                                                                                                                                                                                                      | All columns with numeric types (`np.number`).              |
| `datetime_columns`    | <ul><li>Columns with datetime values.</li><li>Ignored in data drift calculations.</li><li>Affects column stats choice and adds appropriate visualizations.</li></ul>                                                                                                                                                                                                                                                                                                                                                                | All columns with DateTime format (`np.datetime64`).        |
| `categorical_columns` | <ul><li>Columns with categorical values.</li><li>Mapping affects choice of statistics, automated tests, data drift detection methods and visualizations. </li></ul>                                                                                                                                                                                                                                                                                                                                                                 | All other columns not classified as numerical or datetime. |
| `text_columns`        | <ul><li>Text columns.</li><li>You can generate descriptors without explicitly mapping text columns.</li><li>Mapping is useful for statistical analysis (e.g., Data Summary Report). </li><li> Affects choice of automated tests (text columns will be excluded from irrelevant tests), stars and visualizations.</li><li>Unmapped text columns are treated as categorical, which may result in irrelevant evaluations like raw text distribution histograms.</li><li>Mapping always needed for text data drift detection.</li></ul> | Not automated; manual mapping required.                    |
| `embeddings` (TBC)    | <ul><li>Columns containing embeddings.</li><li> Must be specified using a dictionary where keys are embedding names and values are lists of columns.</li><li>Required to use embeddings-specific data drift detection methods.</li></ul>                                                                                                                                                                                                                                                                                            | Not automated; manual mapping required.                    |

**Embedding columns** (TBC).
