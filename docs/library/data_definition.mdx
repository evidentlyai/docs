...
**Manual data definition.** While automation works in many cases, manual mapping is more accurate. It helps avoid mistakes like misclassifying numerical columns with few unique values as categorical. Some evaluations, like text or embedding drift also require explicit mapping.
...
**Tabular or text data**. Map the columns you have in the dataset:
...
<Info>
For **LLM evals**, you can [generate text descriptors](/docs/library/descriptors) without explicitly mapping text columns. But it's often a good idea to map them, since you may later run dataset-level Reports, like to get a Data Summary.
</Info>
...
(TBC). **Embeddings** If you have columns that contain embbeddings
