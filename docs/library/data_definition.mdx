...
- **Column roles** (e.g., id, prediction, target, etc.).

This helps Evidently process the data correctly. Some evaluations require certain columns, and will fail if these are missing. You can create this mapping using the Python API or by mapping the columns visually when you upload the dataset to the Evidently platform.
...
<Info>
  Your data can have [flexible structure](/docs/library/overview#dataset) with any number of categorical, numerical or text columns. You can check the data requirements for specific evaluations in the [Reference table](/metrics/all_metrics). 
</Info>

**Create an Evidently Dataset object** using `Dataset.from_pandas` and pass the corresponding `data_definition`. For automated mapping, you can pass an empty `DataDefinition()`object:
...
<Note>
  This page shows all mapping options. Note that you **only need to use the relevant ones**. For example, you don’t need columns like target/prediction to run data quality or LLM checks.
</Note>
