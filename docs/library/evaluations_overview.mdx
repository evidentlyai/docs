...
This page shows the core evaluation workflow with the open-source Evidently library at glance and links to guides on specific components. 

You can adapt this workflow for all evaluation scenarios (experiments, regression testing, etc.) and use cases: from data to LLM.
...
<Steps>
  ...
  <Step title="Create a Dataset object">
    You must create a Dataset object and set the `DataDefinition()`. This lets you specify column role / types. If empty, Evidently will try guessing the column types. [How to set Data Definition](/docs/library/data_definition).
  ...
  <Step title="(Optional) Add descriptors">
    If you run evals on text data, choose and compute row-level `descriptors` for the Dataset. [How to use Descriptors](/docs/library/descriptors).
