...
<Steps>
  ...
  <Step title="Run evaluations">
    Use `Presets` or configure your own Report. Optionally, add `Tests` to get pass/fail results. There are 100+ built-in evals. You can also add yours. They cover:
...
Here an example Report with Data Quality checks. Other evaluations can be presented in the same way:
...
A `Metric` is a key building block for evaluations. [Built-in Metrics](/metrics/all_metrics) range from simple statistics (like `MeanValue` or `MissingValueCount()`) to complex algorithmic evals (like `DriftedColumnsCount()`). 

Each **Metric** computes a single value and has an optional visual representation (or several to choose from). For convenience, there are also **small Presets** that combine a handful of scores in a single widget, like `ValueStats` that shows many relevant descriptive value statistics at once (varies for column type: categorical, numerical, text).

![](/images/concepts/overview_small_preset_cat_value_compare_example.png)

Similarlity, `DatasetStats` give quick overview of all dataset-level stats, `ClassificationQuality` computes multiple metrics like Precision, Recall, Accuracy, ROC AUC, etc. 
