...
**Using Metrics directly.** You can also compute Metrics directly on the data you have, like a table with ML model logs. You can use Metrics both to understand your data shape and to evaluate different aspects of AI quality like response accuracy.

There are [built-in Metrics](/metrics/all_metrics) that range from simple statistics to complex algorithmic evals. Some examples:

<Tabs>
  <Tab title="MeanValue">
    `MeanValue` computes the mean of any column. \[ADD IMG]
...
A **Report** is an object you create to structure and execute evaluations. You choose the Metrics, run the Report on your dataset, and get a visual summary or simply export the computation result.
...
In many cases, it makes sense to start with **Presets** to simplify the initial setup.

### Metric Presets

Metric Presets are pre-configured evaluation templates. They make it simple to compute multiple related Metrics at once without lengthy setup. ([See all Presets](/metrics/all_presets)).

**Small Presets**. Some Presets are “small” and simply compute and display a handful of Metrics on a single widget. Examples:&#x20;
...
**Comprehensive Presets**. There are also large, more interactive Presets for specific scenarios, like running a set of checks for all columns in the dataset at once. For example:
...
Reports are great for analysis and debugging, or logging metrics. However, in many cases, you don’t want to review all the scores individually but rather confirm that nothing is off.
