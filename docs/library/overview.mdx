...
<Steps>
  ...
  <Step title="Run evaluations">
    Use `Presets` or configure your own Report. Optionally, add `Tests` to get pass/fail results. There are 100+ built-in evals. You can also add yours. Examples:
...
Here an example output for Data Quality checks. Any other evaluation can be presented in the same way:
