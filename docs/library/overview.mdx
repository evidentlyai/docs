...
<Steps>
  ...
  <Step title="Run evaluations">
    Use `Presets` or create a custom evaluation:
...
Once you have the data, you must create a `Dataset` object.&#x20;
...
A **Report** lets you structure and run evaluations on the dataset level. You can generate Reports after computing descriptors, or on any existing dataset like a table with ML model logs.

* Each Report computes and visualizes a set of **Metrics** and **Tests.**&#x20;

* Reports return scores on the **dataset-** and **column**-level.&#x20;

* You can also pass two datasets for a side-by-side comparison. 
...
They help compute multiple related Metrics using a single line of code. Evidently has a number of **comprehensive Presets** ([see all](/metrics/all_presets)) for specific scenarios. They help both understand your data and to evaluate different aspects of AI quality. For example:
...
You can also create your own **custom Report** by listing individual Metrics to include.&#x20;

* You can combine multiple individual Metrics and Presets in a Report. 

* You can include both built-in Metrics and custom Metrics.

A `Metric` is a key building block for evaluations. There are [built-in Metrics](/metrics/all_metrics) that range from simple statistics to complex algorithmic evals. Some examples:
...
Reports are great for analysis and debugging, or logging metrics during monitoring. However, in many cases, you don’t want to review all the scores but run a **conditional check** to confirm that nothing is off.
