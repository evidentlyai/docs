...
A simple example of a descriptor is `TextLength` or `Sentiment`. Descriptors range from deterministic to LLM-based checks. For example, descriptors that use the "LLM judge" approach can label responses as "relevant" or "not relevant" using an evaluation prompt sent to a selected LLM.

Descriptors can also use two texts, such as checking `SemanticSimilarity` between two columns.

You can use built-in Evidently descriptors and configurable templates or add your custom checks in Python. Each Descriptor returns a result that can be:
