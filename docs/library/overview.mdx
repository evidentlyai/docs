...
<Steps>
  <Step title="Pass the input data">
    Pass a dataset with text, tabular or embeddings data. Optionally include a second dataset for comparison.
  </Step>

  <Step title="Run evaluations">
    You can start with evaluation `Presets` or create a custom eval:
...
<Tabs>
  ...
  <Tab title="Data table">
    **Data table**. You can pass any data table to get descriptive stats or run data quality and data drift checks. You can this when evaluating ML model performance without ground truth: include input features and predictions.
