...
That said, we recommend giving this a read to understand the core library components, even if you start with using just one type of evals.
...
<Info>
  **Data sampling**. Running evaluations on large datasets (millions of rows) can take time. The computation time depends on:
...
<Tabs>
  <Tab title="Data Drift">
    Identifies shifts in individual data distribution for all columns in the dataset and consolidates the results.
  ...
  <Tab title="Classification">
    Breaks down classification metrics and includes debugging plots.
  </Tab>
