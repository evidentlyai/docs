...
Evidently has a modular architecture. You can use the evaluation library on its own, or together with the [Monitoring Platform](/docs/platform/overview) (self-hosted or Evidently Cloud). This page gives a closer look at the library. 
...
<Steps>
  ...
  <Step title="Explore results">
    ...
    * **Upload** to Evidently Platform to store and track over time.
    ...
    This exportability makes it easy to integrate Evidently with existing workflows and tools.
  </Step>
</Steps>
...
A simple example of a descriptor is `TextLength` or `Sentiment`. Descriptors range from deterministic to complex ML-based checks. Some let you implement "LLM judges": for example, they can label responses as "relevant" or "not relevant" using an evaluation prompt sent to an LLM.

Descriptors can also use two texts, like checking `SemanticSimilarity` between two columns.

You can use [built-in descriptors](/metrics/all_descriptors), configure templates or add fully custom checks in Python. Each Descriptor returns a result that can be:
