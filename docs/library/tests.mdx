...
<Note>
For a quick end-to-end example of generating Tests, Check the Quickstart [for ML](/quickstart_ml) or [LLM](/quickstart_llm).
</Note>
...
You configure Tests through parameters on the Report or Metric level. Evidently has very flexible API, so you can formulate nuanced conditions and easily setup as many Tests as needed.

### Test Presets

Test Presets automatically generate a set of Tests to evaluate your data or AI system. Each Report Preset has this option. Enable it by setting `include_tests=True`. (Default: False).
...
You can pick such individual Tests, while still using default conditions.

**Select Tests**. Add the individual Metrics to the Report, and choose the the `include_Tests` option:
...
You can define specific pass/fail conditions for each Test. For example, set minimum expected precision or share of a certain category. Tests fail when conditions aren't met.
...
```
Note that we don't need to use `include_tests` when setting Test manually. (It defaults to False).
...
**Additional parameters**. Some Metrics need extra parameters. For example, to check for values outside fixed range, you must set this range:

To test that no value is out of 18-80 range:
...
**Combine custom and default conditions**. You can use both default and custom conditions by setting `include_tests=True` and adding custom conditions where needed. 
...
Your custom conditions override the defaults for those specific Tests.

**Multiple conditions**. You can ddd multiple conditions to the same Metric:
...
This creates two different Tests for the Min value.
...
**Testing against reference**. If you pass a reference dataset, you can set conditions relative to the reference values.

For example, to Test that the number of rows in the current dataset is equal to or up to 10% greater than in reference:
...
This will check that the the number of rows is equal to or at most 5 rows greater than in the reference.

## Set Test criticality

By default, failed Tests return Fail. To get a Warning instead, set `is_critical=False`. For example: 
