...
**Testing against reference**. If you pass a reference dataset, you can set conditions relative to the reference values. For example, to Test that the number of rows in the current dataset is equal to or up to 10% greater than in reference:
...
This checks that the the number of rows is equal to reference or at most 5 rows greater.
