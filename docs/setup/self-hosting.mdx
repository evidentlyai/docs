...
<Info>
  **Evidently Enterprise.** This page explains how to self-host the open-source platform. [Contact us](https://www.evidentlyai.com/get-demo) to get access to the self-hostable Enterprise version with extra features and support.
</Info>

In addition to using Evidently Python library, you can self-host the UI Service to get a monitoring Dashboard and organize the results of your evaluations. This is optional: you can also run evaluations and view results directly in Python or export them to JSON or HTML.
...
Once you [install Evidently](installation), you will need to create a `workspace`. This designates a remote or local directory where you will store the evaluation results (as JSON `snapshots` of the Evidently `Reports`). The UI Service will read the data from this source.
