...
In addition to using Evidently Python library, you can self-host the UI Service to get a monitoring Dashboard and organize the results of your evaluations. This is optional: you can also run evaluations and view results directly in Python or export them to JSON or HTML files.
...
Once you [install Evidently](/docs/installation), you will need to create a `workspace`. This designates a remote or local directory where you will store the evaluation results (JSON Reports called `snapshots`). The UI Service will read the data from this source.
