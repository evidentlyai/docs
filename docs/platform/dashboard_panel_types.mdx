...
<Info>
  **What is the difference between a Distribution Panel (`DashboardPanelHistogram`) and a Histogram Plot (`DashboardPanelPlot` with`PlotType.HISTOGRAM`?** A Histogram shows the distribution of the selected values from all Reports. Each source Report contains a **single value** (e.g., a "mean value"). A Distribution Panel shows how a distribution changes over time. Each source Report contains a **histogram** (e.g. frequency of different categories).
</Info>
