...
<Steps>
  <Step title="Configure evaluations">
    To implement batch evaluations, use the basic flow of the Evidently evaluation library to compute Reports / Test Suites on a schedule. By configuring the Report contents, you can define which metrics to capture and optionally set test conditions.
...
<Tip>
**Running Tests vs Reports**. You can also run Tests on your production data to ensure ongoing quality. Structuring your evaluations as Tests - as opposed to monitoring lots of metrics at once - can help reduce alert fatigue and simplify configuration when evaluating multiple conditions at once. For example, you can quickly verify that all columns in the input data are within a defined min-max range.
</Tip>
