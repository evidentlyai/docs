...
**Value type**. Most Evidently Metrics return a single `value`. For example, `RowCount` returns the row count `value`, and `Accuracy` returns the `accuracy` value. So listing just the `Metric` name is enough.
