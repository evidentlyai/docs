...
<Check>
  Adding Panels in the UI Pro feature available in the Evidently Cloud / Enterprise.
</Check>
...
### `Metric_id` and `metric_fingerprint`

To point to an exact value or Test result to plot on a Panel, you use  `test_filters` or `metric_args `as shown above. The`metric_id` or `metric_fingerprint` you include must corresponds to the Metric that is included in the Reports logged to the Project. You must use the same Metic name (with any parameter) that you used when creating the Report.
...
* `share` or `count` if a Metric can return both absolute count and percentage value (like `MissingValueCount` ) . Applies to Counter and Plot panels.
