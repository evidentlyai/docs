...
To delete or edit a Panel, enter the Edit mode and choose an action by hovering over a specific Panel.
...
**Add a Tab**. You can explicitly create a new empty Tab. Use `create_tab`, give it a name and save the configuration. 
...
**Add a Tab together with a new Panel**. Alternatively, you can use the `add_panel` method as usual and specify the name of the destination Tab. If no there is no Tab with a set name, you will create both a new Tab and Panel at once. If it already exists, a new Panel will appear below others in this Tab.
...
|--------------------------------------|--------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `value: Optional[PanelValue] = None` | `value=PanelValue(metric_args={"metric.metric_id": RowCount().metric_id}, field_path="value", legend="count")` | Specifies the value to display. You must point to a named **Metric** and a specific **result** inside it (`value` or `share`/`count`). If left empty, displays a text-only panel. <br/> </br> *Check the Panel Value section below for examples*.          |
| `text: Optional[str] = None`         | `text="count"`                                                     | Supporting text to display on the Counter.                                                                                                                       |
| `agg: CounterAgg`                    | `agg=CounterAgg.SUM`                                               | Data aggregation options: <br />`SUM`: Calculates the value sum. <br />`LAST`: Displays the last available value. <br />`NONE`: Reserved for text panels. |
