...
To delete/edit a Panel, enter the Edit mode and hover over specific Panel to choose an action.
...
| ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `value: Optional[PanelValue] = None` | Specifies the value to display. <br /> <br />You must point to a named **Metric** and a specific **result** inside it (`value` or `share`/`count`). *Check the Panel Value section below for examples.* <br /><br />If left empty, displays a text-only panel. |
| `text: Optional[str] = None`         | Supporting text to display.                                                                                                                                                                                                                                    |
...
<Tabs>
  ...
  <Tab title="Specific Test (All time)">
    **Specific Test**. To show all failures of specific Test in time. (Filtered by status and Test name).
...
For Metric Panels, you also specify the `field_path`. This helps point to a specific result inside the Metric. This can take the following values:
  ...
  * `value` in cases where a Metric returns a single result (majority). Applies to Counter and Plot panels.

  * `share` or `count` in cases where a Metric (like `MissingValueCount()` ) that return both absolute count and percentage value. Applies to Counter and Plot panels.

  * `values` in cases where the Metric contains a histogram visualization (like `UniqueValueCount()`). Applies to Distribution panel.

<Note>
  **Check Metric results.** You can check the result of each Metric in the [All Metrics table](/metrics/all_metrics). You can also see it if you generate a Report with the Metric you are interested in and export the results as Python dictionary or JSON. 
</Note>
