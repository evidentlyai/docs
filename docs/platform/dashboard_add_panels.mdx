...
**Column (For column-level Metrics / Descriptors)**. When a metric operates at the column level, use the `column` label to specify which column or descriptor it refers to. 
...
**Value type (For Metrics with multiple metric results)**. Most Evidently Metrics return a single `value`. For example, `RowCount` returns the row count `value`, and `Accuracy` returns the `accuracy` value. So listing just the `Metric` name is enough.
...
- Or, generate a local Report with the `metric` and inspect its structure using `report.dict()` or `report.json()`.
</Info>
