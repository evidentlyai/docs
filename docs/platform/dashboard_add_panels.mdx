...
We recommend starting with [pre-built Tabs](/docs/platform/dashboard_tabs) for a quick start. This page describes how you can add panels one by one.
...
<Steps>
  ...
  <Step title="Add a new Panel">
    ...
    * Set applicable Panel **parameters.** (See list for each panel type below).

    * Define the **Panel value(s)** using `metric_id` and `field_path`. (See examples below).&#x20;

    * Add a **legend**.

    * Set if the Panel should appear on specific **Tab**.&#x20;

    For example, to add a line plot that shows row count across all reports:
