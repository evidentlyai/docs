...
**Drift methods**. You can also specify the drift detection methods: set one method for all columns (use `stattest`), by type (use `num_stattest` , `cat_statttest`, `text_stattest`) or individually (use `per_column_stattest`). If some columns are left out, defaults apply.&#x20;
...
You can set the threshold for methods accordingly. For example, to use PSI for categorical columns only, with threshold of 0.5. (If PSI is above, drift is detected for each column).
