...
You can set conditions not only for Tests, but also for Metrics.&#x20;
...
**Drift methods**. You can specify the drift detection methods on the column level. You can set one method for all columns (use `stattest`), by type (use `num_stattest` , `cat_statttest`, `text_stattest`) or individually (use `per_column_stattest`). If some columns are left out, defaults apply.&#x20;
