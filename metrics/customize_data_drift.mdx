...
In this case, if PSI is ≥ 0.3 for any categorical column, drift is detected for that column. Default methods and thresholds apply to other columns (e.g. numerical and text). Dataset drift is detected if over 50% of columns are drifting (default `drift_share` applies).
