...
**Dataset drift share**. This will detect drift if over 70% are drifting, using default methods and thresholds for each column.
...
**Drift methods**. You can also specify the drift detection methods: set one method for all columns, by type or individually.  For example, to use PSI (Population Stability Index) for all columns in the dataset:
...
You can set the threshold for methods accordingly. For example, to use PSI for categorical columns only, with threshold of 0.5.&#x20;
...
If PSI is greater or equal to 0.5 for any categorical column, the drift will be detected for that column. For other columns, default methods / thresholds apply. Overall dataset drift will be detected if over 50% of columns are drifting (defaults apply since nothing else is specified).
...
| `threshold`      | `float`     | The statistical test threshold for drift detection. |
