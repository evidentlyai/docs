...
* **Column drift.** Evaluate drift in each dataset column. The drift detection method is chosen automatically for each column based on type (categorical, numerical, text, embeddings) and number of observations.

* **Overall dataset drift.** Returns the share of drifting columns in the Dataset. By default, Dataset Drift is detected if at least 50% of features drift.

<Info>
  **Data Drift Explainer.** Check the [Data Drift Explainer](/metrics/explainer_recsys) to understand different methods.
</Info>
...
* **To understand model drift in an offline environment.** You can explore the historical data drift to understand past changes in the input data and define the optimal drift detection approach and retraining strategy.
...
* **Input columns.** You can provide any input features and prediction column, if available.

* **Non-empty.** The columns compared for drift must be non-empty.&#x20;

* **Column types.** Evidently can evaluate drift for numerical, categorical, text or embedding data. You can explicitly specify each column type when you run the Report. Otherwise Evidently will try to identify the numerical and categorical features automatically. It is recommended to use data definition to avoid errors. If you have text or embeddings data, you must always specify it.
...
**Select Columns**. You can choose to apply Drift Detection only to some columns in the Dataset, for example, only to the important features. Pass them using the \`columns\` parameter.

**Change Drift Detection conditions.** Evidently has a large number of drift detection methods to choose from (univariate and multivariate), including PSI, K-L divergence, Jensen-Shannon distance, Wasserstein distance, etc. You can specify them or change the overall drift detection conditions by passing parameters to a Preset. You can also pick tests for individual columns or implement a fully custom drift detection method.

<Info>
  **Drift detection parameters**. Check available methods and parameters in [Drift Customization. ](/docs/metrics/customize_data_drift).
</Info>

**Modify Report**. You can add other Metrics to the Report to get a more comprehensive evaluation.  Two recommended options are:

* **Data quality checks**. Add \`DataSummaryPreset\` to get descriptive statistics and run Tests on data quality issues, like detecting missing values. Note that the data drift checks drop the null values (and compare the distributions of non-empty features), so you may want to run such Tests separately.

* **Correlation changes**. You can also consider adding checks on changes in correlations between the features.
