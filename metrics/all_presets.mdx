...
<Note>
  Note that Presets apply on the **dataset level**. If you looking at row-level evaluations (e.g. scoring relevance, correcteness, etc. for LLM outputs), it's best to explore [built-in descriptors](/metrics/all_descriptors) for RAG etc.
</Note>
