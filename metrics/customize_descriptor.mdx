...
You can define a `CustomColumnDescriptor` that can take any column from your dataset and evaluate each row. It will add a single column with numerical (`num`) scores or categorical (`cat`) labels. Implement it as a Python function that takes a Pandas Series as input and return a transformed Series.

For example, to check if the column is empty:
...
You can alternatively define a `CustomDescriptor` that takes one or multiple named columns from your dataset and returns a single column. You can use it for pairwise evaluations.

For example, to check exact match between `target_answer` and `answer` columns:
...
You can also use `CustomDescriptor` to run evals for multiple columns and return multiple scores.

For example, to reverse all words in `question` and `answer` columns:
