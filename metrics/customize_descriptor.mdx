...
You can define a `CustomColumnDescriptor` that can take any column from your dataset and evaluate each row. It will add a single column with numerical (`num`) scores or categorical (`cat`) labels.

Implement it as a Python function that takes a Pandas Series as input and return a transformed Series. 
