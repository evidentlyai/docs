...
* (Optional) **Timestamp.** If you have a DateTime index and want to learn how column values change with time, specify the `timestamp` column in data definition.

* (Optional) **Target**. If you have a target column and want to see feature distribution by target, specify the target column in data definition.
