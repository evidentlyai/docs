...
Evaluations are a core feature of the Evidently library. There 100+ evals with built-in visuals. Before exploring, make sure you know a simple eval workflow (for [LLMs](../quickstart_llm), for [ML](../quickstart_ml)) and understand the key components:

* **Descriptors** evaluate data row-by-row. They return scores (like text length) or labels (like "PII present") for each input.
