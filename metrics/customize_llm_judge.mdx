...
from evidently.features.llm_judge import BinaryClassificationPromptTemplate, MulticlassClassificationPromptTemplate 
...
**Example 2**. This template is very flexible: you can use it for any custom criteria. For instance, you can use to decide if the question is appropriate to the scope of your LLM application. A simplified prompt:
...
### Binary classifier, using multiple columns
...
**Single-column**. Let's evaluate how well the response aligns with the brand policy, classifying into 5 categories. Note that it's generally recommended to use as few categories as possible to make sure each of them is clearly defined.
...
**Multi-column**. Let's evaluate the relevance of answer to the question, classifying into "relevant", "irrelevant" and "partially" relevant. To define the evaluation template:
