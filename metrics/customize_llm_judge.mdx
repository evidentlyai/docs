...
An evaluator can use multiple columns. For example, to assess if the response is grounded in context. To implement this:

* Mention the second `{column_name}` inside your evaluation `criteria.`

* Note: you do **not** need to include the primary column name in the evaluation prompt, since it's already part of the template.
