...
* use built-in evaluators (with pre-written prompts),
...
An evaluator can use multiple columns: for example, to assess if the response is grounded in context.&#x20;

To implement this, mention the second `{column_name}` inside your evaluation criteria.
...
You do not need to include the primary column name in the evaluation prompt, since it's already but of the template.

When applying the descriptor, include the second column using the `additional_columns` parameter:

```
eval_df.add_descriptors(descriptors=[
    LLMEval("response", template=hallucination, provider = "openai", model = "gpt-4o-mini", alias="appropriate_q", additional_columns={"context": "context"}),
])
```
