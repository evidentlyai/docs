...
`RecsysPreset` evaluates the quality of the recommender system by generating multiple metrics to assess the quality of ranking and diversity of recommended content.

It include 10+ metrics like NDCG at K, MAP at K, HitRate, diversity, serendipity, etc. You must provide a `k` parameter to evaluate the Top-K recommendations.
...
<Info>
  **Preset composition.** Verify the exact Preset composition at the [All Metrics reference page](/docs/metrics/all-metrics).
</Info>

<Info>
  **Metric explainers.** Check the [Ranking Metrics Explainer](/metrics/explainer_recsys) to see how each Metric works.
</Info>
...
**Metrics**. If you want to change the Metric composition, create a custom Report listing the individual Metrics. This is straightforward. You can also implement fully custom Metrics.
