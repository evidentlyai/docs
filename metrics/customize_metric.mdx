...
You can build fully custom Metrics/Tests to handle any column- or dataset-level evaluations. This lets you implement business metrics, weighted scores, and more.
...
* (Optional). Defining the default Test conditions that apply when you run Tests for this Metric (with or without Reference).
...
The default Test will checks if the max value is 0 (or within ±10% of the reference value). You can also set any custom threshold when calling the Metric.
