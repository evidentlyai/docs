...
The last eval uses LLM-as-a-judge with a template Evidently prompt. It returns a label with an explanation. You will need an OpenAI token (eval defaults to `gpt-4o-mini`). 
...
**Alternative**. If you don't want to use LLM judge, you can run a deterministic eval: check if words "sorry" or "apologize" are present (True/False).
