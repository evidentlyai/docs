...
<Tabs>
  <Tab title="Evidently Cloud">
    ...
    Prepare your data as a pandas dataframe. Here’s a toy chatbot dataset with "Questions" and "Answers".
...
**Collecting live data**: you can also trace inputs and outputs from your LLM app, and download the dataset created from traces for evals. Check the [Tracing Quickstart](cloud_quickstart_tracing.md). 
</Callout>
...
The last eval uses LLM-as-a-judge using a template Evidently prompt. It returns a label with an explanation. You will need an OpenAI token (eval defaults to `gpt-4o-mini`). 
...
**Alternative**. To run evals that work locally, let's switch the LLM judge for determinstic check: if words "sorry" or "apologize" are present (return True/False).
    ...
    Each evaluation is a `descriptor`. You can choose from multiple built-in evaluations or create custom ones.
