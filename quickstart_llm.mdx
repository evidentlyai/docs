...
You will run a simple evaluation in Python and explore results in Evidently Cloud. For a fully local workflow, skip Step 1.
...
<Info>
  **Preparing data**. You can provide data in any structure. Some common setups:
...
- **Denials:** chatbot refusals, using an LLM-as-a-judge with built-in prompt.

Each evaluation adds a `descriptor` to your dataset that appears as a new column.
...
<Note>
  No OpenAI key? There is an non-LLM alternative below.
</Note>
...
Pass the dataset and specify the list of `descriptors` to add:
...
To use a keyword-based check instead of LLM judge, pass `IncludesWords` as shown.
...
Create a Report to summarize the evaluation results:
...
You can upload this to the Evidently Cloud or view locally.

**Upload results to Evidently Cloud** with raw data:

```python
ws.add_run(project.id, my_eval, include_data=True)
```

Now you can go to [Evidently Cloud](https://app.evidently.cloud/), open your Project, and navigate to Reports. You will see all score summaries and browse the data. E.g. sort to find all answers labeled as "Denials".

![](/images/examples/llm_quickstart_explore.png)

**Get a Dashboard**. As you run repeated evals, it's useful to track them over time. Go to the "Dashboard" in the left menu, enter the "Edit" mode, and add a new "Columns" tab:

![](/images/examples/llm_quickstart_create_tab_new.gif)

You'll see a panels that show descriptor values. Each will have a single data point for now. As you log more evaluation results, you can track trends and set up alerts.

**Local preview**. To view the Report in the Python environment like Jupyter notebook or Colab, run:

```python
my_eval
```

You can also view the results as a JSON or Python dictionary, or save it as an external HTML file.
...
- **Text length** is at most 150 symbols (less or equal t.
- **Denials**: there are none.
- If any condition is false, consider the output to be a "fail".

You can implement this through extra parameters.
