...
* **Denials**: detect if the chatbot denied an answer. This eval uses LLM-as-a-judge with a template Evidently prompt (defaults to `OpenAI` and `gpt-4o-mini`). It returns a label with an explanation.
...
<Info>
  **Alternative**. If you don't have an OpenAI key, you can run a deterministic eval that checks if words "sorry" or "apologize" are present in text (True/False). Use the `IncludesWords` as shown in the code comment.
</Info>
...
<Tabs>
  <Tab title="Evidently Cloud">
    ...
    **See Test results**. Inside the "Tests" tab of the Report, you will get a pass/fail summary for your conditions. 
  ...
  <Tab title="Local">
    You can view the Report directly in an interactive Python environment like Jupyter notebook or Colab. Run:
