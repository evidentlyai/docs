...
[**Evidently**](https://github.com/evidentlyai/evidently) is a popular open-source library with over 25 million downloads, 5000+ GitHub stars, and a thriving community. It's licensed under Apache 2.0. This gives full transparency - you can see exactly how every metric works and trust the implementation. It also delivers an intuitive API designed for a great developer experience.
...
Evidently is built to adapt to your needs without lock-ins or complex setups. It’s modular and component-based, so you can with only what you need. You don't have to deploy a service with multiple databases just to run a single eval.
...
Many other tools provide a system to run and log evals, but make you implement all metrics from scratch. We ship **100+ built-in evaluations** that cover many ML and LLM use cases. From ranking metrics to data drift algorithms and LLM judges, we’ve done the hard work by implementing metrics and ways to visualize them.  
