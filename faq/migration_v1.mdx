...
This guide explains the key changes introduced in Evidently 0.6. It outlines key differences and highlights the advantages of the new implementations.

These are breaking changes that are currently available when you import components from `evidently.future`. **This new API will become the default in the upcoming 1.0 release**. 

If you're new to Evidently, skip this page and head directly to the Quickstart for [ML](/quickstart_ml) or [LLM](/quickstart_llm).

<Note>
You can still access the previous Evidently API inside versions 0.6 and later, so you have time to make the switch.
</Note>  
...
Descriptors provide row-level text evaluations, ranging from basic checks (e.g., text length) to LLM-based evals (e.g., checking for contradictions). 
...
**1. Compute Descriptors**. Add them to the source table containing inputs and outputs. You can do this together with data definition.
...
**2. Aggregate results or run conditional checks**. Use these descriptors like any other dataset column when creating a Report.

Here is how you summarize all descriptors and check that the text length is under 100 symbols.
...
As you have probably noticed in the example above, we also made the changes to the core Report API. Here is how generating a single Report with data summary preset for a single dataset works now:
...
You can also now use "Group by" to compute metrics for specific segments.

## Test Suites joined with Reports

Most importantly,  Reports and Tests are now unified. Previously, these were separate:
* Reports provided a visual overview of metrics (e.g., distribution summaries, statistics).
* Tests let you set pass/fail conditions (e.g., checking for missing data or LLM quality thresholds).
...
This redesign significantly improves JSON result parsing and UI integration.

You can check the list of new Metrics here. To get a pre-built combination of multiple checks at once, you can still use Presets.

<Card title="Metric" href="/metrics/all-metrics">
  All Metrics.
</Card>

## Simplified Dashboard API

With the redesigned Metrics, the Dashboard API is now much simpler. You can create new panels and point to specific Metric results with a strictly fixed set of options.
