...
Evidently 0.6 introduces an update to the core API with breaking changes. 

Currently the new API is available when you import components from `evidently.future`. **This API will become the default in the upcoming 1.0 release**. 

<Note>
You can still access the earlir Evidently API inside versions 0.6 and later.
</Note>  
