...
* **Get new answers**
  ...
  . Imitate generating new answers to the same question.

* **Create and run a Report with Tests**

  . Compare the answers using LLM-as-a-judge to evaluate length, correctness and style consistency.
...
* **Correctness**
  ...
  . All new responses should not contradict the reference answer.
...
<Info>
  **Ideally, evaluate your judge first!** Each LLM evaluator is a small ML system you should align with your preferences. We recommend running a couple of iterations. Check the tutorial on [LLM judges](/examples/LLM_judge.md).
</Info>
...
* To set test conditions, you define the expectations using parameters like
  ...
  [Test docs](/docs/library/tests)

  ).

* We also label one of the tests (style match) as non-critical. This means it will trigger warning instead of a fail, and will be visually labeled yellow in the Report o a monitoring panel.
