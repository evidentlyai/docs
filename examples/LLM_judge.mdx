...
* How to evaluate text for custom criteria using LLM as the judge.

* How to evaluate the quality of LLM judge against your own labels.
...
* **Reference-based**
  ...
  . Compare new responses against a reference. This is useful for regression testing or whenever you have a "ground truth" (approved responses) to compare against.

* **Open-ended**

  . Evaluate responses based on custom criteria, which helps evaluate new outputs when there's no reference available.

Our primary focus is to demonstrate **how to create and tune the LLM evaluator**, which you can then apply in different contexts, like regression testing or prompt comparison.
...
* **Create an evaluation dataset**
  ...
  . Create a toy Q&A dataset with two responses to each question.

* **Create and run an LLM as a judge**

  . Design an LLM evaluator prompt.

* **Evaluate the judge**

  . Compare the LLM judge's evaluations with manual labels.
...
<Info>
  Use the provided code snippets or run a sample notebook: [Jupyter notebook](https://github.com/evidentlyai/community-examples/blob/main/tutorials/LLM_as_a_judge_tutorial_updated.ipynb) or [open it in Colab](https://colab.research.google.com/github/evidentlyai/community-examples/blob/main/tutorials/LLM_as_a_judge_tutorial_updated.ipynb).
</Info>
...
* **Questions**
  ...
  . The inputs sent to the LLM app.

* **Target responses**

  . The approved responses you consider accurate.

* **New responses**

  . Imitated new responses from the system.

* **Manual labels with explanation**

  . Labels that say if response is correct or not.

Why add the labels? It's a good idea to be the judge yourself before you write a prompt. This helps:&#x20;

* Formulate better criteria. You discover nuances you may want to clarify in the evaluator prompt.
...
**Data definition**. To evaluate the classification quality, we need to map the structure of the dataset. The column with the manual label is the "target", and the LLM-judge response is the "prediction":
...
<Info>
  `Pos_label` in refers to the class that is treated as the target ("what we want to predict better") for metrics like precision, recall, F1-score.
</Info>
...
We can now get a more balanced evaluation and explore the confusion matrix. We have one type of error each so overall the results are pretty good! You can also continue refining your judge prompt.

![](/images/examples/llm_judge_tutorial_conf_matrix-min.png)

## 5. Verbosity evaluator

Next, let’s create a simpler LLM judge that evaluates the verbosity of the responses. This judge will check whether the responses are concise and to the point. This only requires evaluating one output column. This is perfect for production evaluations where you don’t have a reference answer.
