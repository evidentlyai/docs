---
title: 'RAG evals'
description: 'Simple example of using RAG-related metrics.'
---
...
* **Retrieval evaluation.** Assessing the quality of retrieved contexts, including per-chunk relevance.
* **Generation evaluation.** Evaluating the quality of the final response, both with and without ground truth.

By the end of this tutorial, you'll know to evaluate different aspects of RAG system, and generate structured reports to track RAG performance.
...
```python
synthetic_data = [
    ["Why do flowers bloom in spring?", "Plants require extra care during cold months.", "because of the rising temperatures"],
    ["Why do we yawn when we see someone else yawn?", "Yawning is contagious due to social bonding and mirror neurons.", "because it's a glitch in the matrix"],
    ["How far is Saturn from Earth?", "Saturn is about 1.4 billion kilometers away.", "about 1.4 billion kilometers"],
    ["Where do penguins live?", "Penguins primarily live in the Southern Hemisphere.", "mostly in Antarctica"]
]
...
* Simulatenously, we add [descriptors](/docs/library/descriptors): evaluators that score each row.
* We use a built-in LLM judge metric. You can further customize it by choosing a different evaluator LLM or modifying the prompt. [Custom LLM judge parameters](/metrics/customize_llm_judge).
...
context_based_evals = Dataset.from_pandas(
    synthetic_df,
    data_definition=DataDefinition(text_columns=["Question", "Context", "Response"]),
    descriptors=[ContextRelevance("Question", "Context", output_scores=True, aggregation_method="hit", method="llm", alias="Hit")]
)
context_based_evals.as_dataframe()
```
...
RAG systems often retrieve multiple chunks. In this case, we can assess the relevance of each indiviudal chunk first. 
...
If you output scores, you can also see the list of relevance scores that appear in the same order as your chunks. 
...
context_based_evals = Dataset.from_pandas(
    synthetic_df_2,
    data_definition=DataDefinition(text_columns=["Question", "Context", "Response"]),
    descriptors=[ContextRelevance("Question", "Context", output_scores=True, aggregation_method="mean", method="llm", alias="Relevance")]
)
context_based_evals.as_dataframe()
```
...
## 4. Evaluating Generation 

### With Ground Truth

If you a have ground truth dataset for RAG, you can compare generated responses against known correct answers.

<Note>
  **Synthetic data**. You can also generate example inputs with correct outputs for your RAG system using [Evidently Platform](/docs/platform/datasets_generate).
</Note>
...
<Note>
  **Changing the LLM prompt**. You can tweak the definition of correctness to your own liking. Here is an example tutorial on how we tune [a correctness descriptor prompt](/examples/LLM_judge).
</Note>

### Without Ground Truth

If you don't have reference answers, you can use reference-free LLM judges to assess response quality.

For example, here is you can run evaluation for `Faithfulness` to detect if the response is contradictory or unfaithful to the context:
...
Once you defined what you are evaluating, you can group all your evals in a Report to summarize the results across multiple tested inputs. 

Let's put it all together. 

**Score data**. Once you have a pandas dataframe `synthetic_df`, you create an Evidently dataset object and choose the selected decsriptors by simply listing them.
...
This lets you see a well-rounded evaluation. In this toy example, we can se that the the system retrieves the right data better than uses it for generation. The next step could be improving your generation prompt to ensure responses stay true to conetxt.
...
report = Report([
    TextEvals(),
    CategoryCount(column="Faithfulness", category="FAITHFUL", tests=[eq(0)]),
...
In this case, we expect all retrieved contexts to be valid and all responses to be faithful, so our test fail. You can adjust these conditions — for example, allowing a certain percentage of responses to fail. 

![](/images/examples/rag_tests-min.png)

## 6. Upload to Evidently Cloud
