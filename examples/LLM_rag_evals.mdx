...
<Info>
We’ll demonstrate a **local workflow** using an open-source Python library, exporting results to a pandas dataframe and generating a report in a notebook—ideal for Jupyter or Colab. At the end, we also show how to upload results to the Evidently Platform. If you are in a non-interactive Python environment, choose this option.
</Info>
